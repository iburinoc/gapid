...
func dependencySync(ctx context.Context, d *sync.Data, c *path.Capture) error {
	...
	addPoint := func(pt sync.SyncPoint) sync.SyncIdx {
		d.SyncPoints = append(d.SyncPoints, pt)
		return sync.SyncIdx(len(d.SyncPoints) - 1)
	}
	...
	getCmdPoint := func(id api.CmdID) sync.SyncIdx {
		if pt, ok := d.CmdSyncPoints[id]; ok {
			return pt
		}
		d.CmdSyncPoints[id] = addPoint(sync.CmdIdx{[]uint64{uint64(id)}})
		return d.CmdSyncPoints[id]
	}
