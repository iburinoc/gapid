...
func (ds *descriptorSet) writeDescriptors(ctx context.Context,
	...
	case VkDescriptorType_VK_DESCRIPTOR_TYPE_SAMPLER,
		...
		VkDescriptorType_VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT:
		for _, imageInfo := range write.PImageInfo().Slice(0, count, l, nil).MustRead(ctx, cmd, s, nil) {
	...
	case VkDescriptorType_VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,
		...
		for _, bufferInfo := range write.PBufferInfo().Slice(0, count, l, nil).MustRead(ctx, cmd, s, nil) {
			updateDstForOverflow()
	...
	case VkDescriptorType_VK_DESCRIPTOR_TYPE_UNIFORM_TEXEL_BUFFER,
		VkDescriptorType_VK_DESCRIPTOR_TYPE_STORAGE_TEXEL_BUFFER:
		for _, vkBufView := range write.PTexelBufferView().Slice(0, count, l, nil).MustRead(ctx, cmd, s, nil) {
			updateDstForOverflow()
...
func (vb *FootprintBuilder) recordBarriers(ctx context.Context,
	...
	if memoryBarrierCount > 0 {
		...
		for _, barrier := range pBufferBarriers.Slice(0,
			uint64(bufferBarrierCount), l, nil).MustRead(ctx, cmd, s, nil) {
		...
		for _, barrier := range pImageBarriers.Slice(0,
			uint64(imageBarrierCount), l, nil).MustRead(ctx, cmd, s, nil) {
...
func (vb *FootprintBuilder) BuildFootprint(ctx context.Context,
	...
	case *VkFlushMappedMemoryRanges:
		...
		for _, rng := range cmd.PMemoryRanges().Slice(0, count, l, nil).MustRead(ctx, cmd, s, nil) {
	...
	case *VkCreateSharedSwapchainsKHR:
		...
		for _, vkSw := range cmd.PSwapchains().Slice(0, count, l, nil).MustRead(ctx, cmd, s, nil) {
			write(ctx, bh, vkHandle(vkSw))
		}
	...
	case *VkGetSwapchainImagesKHR:
		...
		if cmd.PSwapchainImages() == 0 {
			...
			for _, vkImg := range cmd.PSwapchainImages().Slice(0, count, l, nil).MustRead(ctx, cmd, s, nil) {
				write(ctx, bh, vkHandle(vkImg))
				vb.images[vkImg] = newImageLayoutAndData(ctx, bh)
	...
	case *VkQueuePresentKHR:
		...
		for _, vkSp := range info.PWaitSemaphores().Slice(0, spCount, l, nil).MustRead(ctx, cmd, s, nil) {
			if read(ctx, bh, vkHandle(vkSp)) {
				read(ctx, bh, vb.semaphoreSignals[vkSp])
			}
		}
		swCount := uint64(info.SwapchainCount())
		imgIds := info.PImageIndices().Slice(0, swCount, l, nil)
		for swi, vkSw := range info.PSwapchains().Slice(0, swCount, l, nil).MustRead(ctx, cmd, s, nil) {
	...
	case *VkCreateDescriptorSetLayout:
		...
		info := cmd.PCreateInfo().MustRead(ctx, cmd, s, nil)
		bindings := info.PBindings().Slice(0, uint64(info.BindingCount()), l, nil).MustRead(ctx, cmd, s, nil)
		for _, b := range bindings {
			if b.PImmutableSamplers() != memory.Nullptr {
				samplers := b.PImmutableSamplers().Slice(0, uint64(b.DescriptorCount()), l, nil).MustRead(ctx, cmd, s, nil)
	...
	case *VkAllocateDescriptorSets:
		...
		vkLayouts := info.PSetLayouts().Slice(0, setCount, l, nil)
		for i, vkSet := range cmd.PDescriptorSets().Slice(0, setCount, l, nil).MustRead(ctx, cmd, s, nil) {
	...
	case *VkFreeDescriptorSets:
		...
		for _, vkSet := range cmd.PDescriptorSets().Slice(0, count, l, nil).MustRead(ctx, cmd, s, nil) {
	...
	case *VkCreatePipelineLayout:
		...
		for _, setLayout := range info.PSetLayouts().Slice(0, setCount, l, nil).MustRead(ctx, cmd, s, nil) {
	...
	case *VkCreateGraphicsPipelines:
		...
		for _, info := range cmd.PCreateInfos().Slice(0, infoCount, l, nil).MustRead(ctx, cmd, s, nil) {
			stageCount := uint64(info.StageCount())
			for _, stage := range info.PStages().Slice(0, stageCount, l, nil).MustRead(ctx, cmd, s, nil) {
				module := stage.Module()
		...
		for _, vkPl := range cmd.PPipelines().Slice(0, infoCount, l, nil).MustRead(ctx, cmd, s, nil) {
	...
	case *VkMergePipelineCaches:
		...
		for _, src := range cmd.PSrcCaches().Slice(0, srcCount, l, nil).MustRead(ctx, cmd, s, nil) {
	...
	case *VkCreateFramebuffer:
		...
		for _, att := range info.PAttachments().Slice(0, attCount, l, nil).MustRead(ctx, cmd, s, nil) {
	...
	case *VkAllocateCommandBuffers:
		...
		for _, vkCb := range cmd.PCommandBuffers().Slice(0, count, l, nil).MustRead(ctx, cmd, s, nil) {
	...
	case *VkCmdCopyImage:
		...
		for _, region := range cmd.PRegions().Slice(0, count, l, nil).MustRead(ctx, cmd, s, nil) {
	...
	case *VkCmdClearColorImage:
		...
		overwritten := false
		for _, rng := range cmd.PRanges().Slice(0, count, l, nil).MustRead(ctx, cmd, s, nil) {
	...
	case *VkCmdBindVertexBuffers:
		...
		offsets := cmd.POffsets().Slice(0, count, l, nil).MustRead(ctx, cmd, s, nil)
		subBindings := []resBindingList{}
		for i, vkBuf := range cmd.PBuffers().Slice(0, count, l, nil).MustRead(ctx, cmd, s, nil) {
	...
	case *VkCmdClearAttachments:
		...
		rects := []VkClearRect{}
		for _, att := range cmd.PAttachments().Slice(0, attCount, l, nil).MustRead(ctx, cmd, s, nil) {
			atts = append(atts, att)
		}
		for _, rect := range cmd.PRects().Slice(0, rectCount, l, nil).MustRead(ctx, cmd, s, nil) {
	...
	case *VkCmdWaitEvents:
		...
		for _, vkEv := range cmd.PEvents().Slice(0, evCount, l, nil).MustRead(ctx, cmd, s, nil) {
	...
	case *VkCmdExecuteCommands:
		...
		count := uint64(cmd.CommandBufferCount())
		for _, vkScb := range cmd.PCommandBuffers().Slice(0, count, l, nil).MustRead(ctx, cmd, s, nil) {
	...
	case *VkQueueSubmit:
		...
		hasCmd := false
		for i, submit := range cmd.PSubmits().Slice(0, submitCount, l, nil).MustRead(ctx, cmd, s, nil) {
			commandBufferCount := uint64(submit.CommandBufferCount())
			for j, vkCb := range submit.PCommandBuffers().Slice(0, commandBufferCount, l, nil).MustRead(ctx, cmd, s, nil) {
				read(ctx, bh, vkHandle(vkCb))
			...
			for _, vkSp := range submit.PWaitSemaphores().Slice(0, waitSemaphoreCount, l, nil).MustRead(ctx, cmd, s, nil) {
			...
			for _, vkSp := range submit.PSignalSemaphores().Slice(0, signalSemaphoreCount, l, nil).MustRead(ctx, cmd, s, nil) {
	...
	case *VkQueueBindSparse:
		read(ctx, bh, vkHandle(cmd.Queue()))
		for _, bindInfo := range cmd.PBindInfo().Slice(0, uint64(cmd.BindInfoCount()), l, nil).MustRead(
			ctx, cmd, s, nil) {
			for _, bufferBinds := range bindInfo.PBufferBinds().Slice(0,
				uint64(bindInfo.BufferBindCount()), l, nil).MustRead(ctx, cmd, s, nil) {
				if read(ctx, bh, vkHandle(bufferBinds.Buffer())) {
					buf := bufferBinds.Buffer()
					binds := bufferBinds.PBinds().Slice(0, uint64(bufferBinds.BindCount()), l, nil).MustRead(
						ctx, cmd, s, nil)
			...
			for _, opaqueBinds := range bindInfo.PImageOpaqueBinds().Slice(0,
				uint64(bindInfo.ImageOpaqueBindCount()), l, nil).MustRead(ctx, cmd, s, nil) {
				if read(ctx, bh, vkHandle(opaqueBinds.Image())) {
					img := opaqueBinds.Image()
					binds := opaqueBinds.PBinds().Slice(0, uint64(opaqueBinds.BindCount()), l, nil).MustRead(
						ctx, cmd, s, nil)
					for _, bind := range binds {
						if read(ctx, bh, vkHandle(bind.Memory())) {
			...
			for _, imageBinds := range bindInfo.PImageBinds().Slice(0,
				uint64(bindInfo.ImageBindCount()), l, nil).MustRead(ctx, cmd, s, nil) {
				if read(ctx, bh, vkHandle(imageBinds.Image())) {
					img := imageBinds.Image()
					binds := imageBinds.PBinds().Slice(0, uint64(imageBinds.BindCount()), l, nil).MustRead(
						ctx, cmd, s, nil)
					for _, bind := range binds {
						if read(ctx, bh, vkHandle(bind.Memory())) {
	...
	case *VkWaitForFences:
		...
		for _, vkFe := range cmd.PFences().Slice(0, fenceCount, l, nil).MustRead(ctx, cmd, s, nil) {
			if read(ctx, bh, vkHandle(vkFe)) {
				read(ctx, bh, vb.fences[vkFe].signal)
				read(ctx, bh, vb.fences[vkFe].unsignal)
				bh.Alive = true
