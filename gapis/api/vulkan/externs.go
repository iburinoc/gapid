...
func (e externs) hasDynamicProperty(info VkPipelineDynamicStateCreateInfoᶜᵖ,
	...
	l := e.s.MemoryLayout
	dynamicStateInfo := info.Slice(0, 1, l, nil).MustRead(e.ctx, e.cmd, e.s, e.b)[0]
	states := dynamicStateInfo.PDynamicStates().Slice(0, uint64(dynamicStateInfo.DynamicStateCount()), l, nil).MustRead(e.ctx, e.cmd, e.s, e.b)
...
func (e externs) readMappedCoherentMemory(memoryHandle VkDeviceMemory, offsetInMapped uint64, readSize memory.Size) {
	...
	for _, r := range writeRngList {
		mem.Data().Slice(dstStart+r.Base, dstStart+r.Base+r.Size).
			Copy(e.ctx, U8ᵖ(mem.MappedLocation()).Slice(srcStart+r.Base, srcStart+r.Base+r.Size, l, nil), e.cmd, e.s, e.b)
	}
}
...
func (e externs) numberOfPNext(pNext Voidᶜᵖ) uint32 {
	...
	for pNext != 0 {
		counter++
		pNext = Voidᶜᵖᵖ(pNext).Slice(1, 2, l, nil).MustRead(e.ctx, e.cmd, e.s, e.b)[0]
	}
	return counter
}
