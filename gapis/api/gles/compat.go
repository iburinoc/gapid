	...
	t = transform.Transform("compat", func(ctx context.Context, id api.CmdID, cmd api.Cmd, out transform.Writer) {
		...
		case *GlVertexAttrib4fv:
			if c.Vertex().Attributes().Contains(cmd.Location()) {
				...
				cmd.Mutate(ctx, id, s, nil /* no builder, just mutate */, nil)
		...
		case *GlDrawBuffers:
			...
			bufs := cmd.Bufs().Slice(0, uint64(cmd.N()), s.MemoryLayout, nil).MustRead(ctx, cmd, s, nil)
		...
		case *GlDeleteBuffers:
			cmd.Extras().Observations().ApplyReads(s.Memory.ApplicationPool())
			ids, err := cmd.Buffers().Slice(0, uint64(cmd.Count()), s.MemoryLayout, nil).Read(ctx, cmd, s, nil)
			if err == nil {
		...
		case *GlDeleteFramebuffers:
			...
			cmd.Extras().Observations().ApplyReads(s.Memory.ApplicationPool())
			fbs, err := cmd.Framebuffers().Slice(0, uint64(cmd.Count()), s.MemoryLayout, nil).Read(ctx, cmd, s, nil)
			if err == nil {
				for _, fb := range fbs {
		...
		case *GlDeleteProgramPipelines:
			cmd.Extras().Observations().ApplyReads(s.Memory.ApplicationPool())
			ids, err := cmd.Pipelines().Slice(0, uint64(cmd.N()), s.MemoryLayout, nil).Read(ctx, cmd, s, nil)
			if err == nil {
		...
		case *GlDeleteQueries:
			cmd.Extras().Observations().ApplyReads(s.Memory.ApplicationPool())
			ids, err := cmd.Queries().Slice(0, uint64(cmd.Count()), s.MemoryLayout, nil).Read(ctx, cmd, s, nil)
			if err == nil {
		...
		case *GlDeleteRenderbuffers:
			cmd.Extras().Observations().ApplyReads(s.Memory.ApplicationPool())
			ids, err := cmd.Renderbuffers().Slice(0, uint64(cmd.Count()), s.MemoryLayout, nil).Read(ctx, cmd, s, nil)
			if err == nil {
		...
		case *GlDeleteSamplers:
			cmd.Extras().Observations().ApplyReads(s.Memory.ApplicationPool())
			ids, err := cmd.Samplers().Slice(0, uint64(cmd.Count()), s.MemoryLayout, nil).Read(ctx, cmd, s, nil)
			if err == nil {
		...
		case *GlDeleteTextures:
			cmd.Extras().Observations().ApplyReads(s.Memory.ApplicationPool())
			ids, err := cmd.Textures().Slice(0, uint64(cmd.Count()), s.MemoryLayout, nil).Read(ctx, cmd, s, nil)
			if err == nil {
		...
		case *GlDeleteTransformFeedbacks:
			cmd.Extras().Observations().ApplyReads(s.Memory.ApplicationPool())
			ids, err := cmd.Ids().Slice(0, uint64(cmd.Count()), s.MemoryLayout, nil).Read(ctx, cmd, s, nil)
			if err == nil {
		...
		case *GlDeleteVertexArrays:
			cmd.Extras().Observations().ApplyReads(s.Memory.ApplicationPool())
			ids, err := cmd.Arrays().Slice(0, uint64(cmd.Count()), s.MemoryLayout, nil).Read(ctx, cmd, s, nil)
			if err == nil {
